...
# define topics
mydict= {0: 'Trip Visit', 1: 'Trading Bussiness', 2: 'Power Sector', 3: 'Contract Documentation',
         4: 'Mode of Communication', 5: 'Team members discussion', 6: 'Project and Aggreement', 
         7: 'Document Sharing', 8: 'Meeting Schedule', 9: 'Others', 10: 'Others', 11: 'Bussiness Deal', 12: 'Fun Plans',
         13: 'Personal Discussions', 14: 'Management Decisions', 15: 'Finance',
         16: 'Banking'}
...
f = open(r'C:\nlp_project\LDA_Bert_Topic_modeling\working_with_nonames\LDA_BERT_16_Topics_nonames\LDA_BERT_2020_05_22_00_04_53/LDA_BERT_2020_05_22_00_04_53_seperate.file', 'rb')
model_tm = pickle.load(f)

print(model_tm.cluster_model.labels_)
...
# function used for preprocessing called in "preprocess"
...
def preprocess(docs, samp_size=None):
    ...
    #     print(samp)
    samp =  list(range(0, samp_size)) # changed by saher
...
def get_prediction_tm (model_lb, sentences, token_lists):
    # out_of_sample = None
    # out_of_sample = out_of_sample is not None
    # if out_of_sample:
    method = "LDA_BERT"
    corpus = [model_lb.dictionary.doc2bow(text) for text in token_lists]
    if method != 'LDA':
        vec = model_lb.vectorize(sentences, token_lists)
    #         print(vec)
    else:
        corpus = corpus
        vec = model_lb.vec.get(method, None)
    #         print(vec)
    
    if method == "LDA":
        lbs = np.array(list(map(lambda x: sorted(model_lb.ldamodel.get_document_topics(x), 
                                                 key=lambda x: x[1], reverse=True)[0][0], corpus)))
    else:
        lbs = model_lb.cluster_model.predict(vec)
    return lbs
...
def main(text):
    ...
    result = mydict[results_tm[0]]
    print (result)
    print("DONE")
    file1 = open('topic_model_result.txt', 'w')
    file1.write(result)
    file1.close()
    
from pathlib import Path
txt = Path('topic_model_text.txt').read_text()
print(txt)
main(txt)